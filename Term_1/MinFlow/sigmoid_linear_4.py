...
class Linear(Node):
    ...
    def forward(self):
        ...
        input_matrix_X = self.inbound_nodes[0].value
        weight_matrix_W = self.inbound_nodes[1].value
        vector_bias_b = self.inbound_nodes[2].value

        self.value = np.dot(input_matrix_X, weight_matrix_W) + vector_bias_b
...
class Sigmoid(Node):
    def __init__(self, sigmoid):
        Node.__init__(self, [sigmoid])

    def _sigmoid(self):
        linear = self.inbound_nodes[0].value

        base = np.add(1, np.exp(-linear))  # (1-e^x)

        return np.divide(1, base)  # 1/base
